...
export default function SeleccionarProductos() {
  ...
  // 1. Cambiamos el estado para guardar un objeto de selecciones
  ...
  // 2. El manejador de cambios ahora actualiza la categoría específica
  ...
  // 3. La lógica para agregar al carrito ahora añade todos los productos seleccionados
  const handleAddToCart = (e) => {
    e.preventDefault();
    const productosSeleccionados = Object.values(selecciones).filter(Boolean); // Filtra selecciones vacías
    ...
    // Limpiar selecciones y cantidad
    setSelecciones({});
    setCantidad(1);
  };
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-800 rounded-lg shadow-md">
      ...
      <form onSubmit={handleAddToCart} className="flex flex-col gap-4">
        ...
        {/* 4. El JSX se actualiza para usar el nuevo estado y manejador */}
        {Object.keys(productosAgrupados).map((categoria) => (
          <div key={categoria}>
            ...
            <select
              value={selecciones[categoria] || ""} // Lee del objeto de selecciones
              onChange={(e) => handleSelectionChange(e, categoria)} // Pasa la categoría al manejador
        ...
        <button 
          type="submit" 
          disabled={Object.values(selecciones).every(v => !v)} // Deshabilita si no hay nada seleccionado
